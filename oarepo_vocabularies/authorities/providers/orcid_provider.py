...
class ORCIDProvider(AuthorityProvider):
    ...
    def search(self, identity, params, **kwargs):
        ...
        results = self.orcid_client.search(params.get("q", ""), method="edismax", start=params.get("page", 1), rows=params.get("page_size", 20), access_token=access_token)
    ...
    def to_vocabulary_item(orcid_item):
        ...
        for key, activity in activities_summary.items():
            if key == "path" or key == "last-modified-date":
                continue
            
            affiliation_group = activity.get("affiliation-group", [])
            for affiliation in affiliation_group:
                affiliation_summary = affiliation.get("summaries", [{}])[0]
                
                affiliation_values = next(iter(affiliation_summary.values()), None)

                organization = affiliation_values.get("organization", {})
...
# 
    ...
    # orcid_client = ORCIDClient(client_id, client_secret, testing=True)

    # get the access token
    # access_token = orcid_client.get_search_token_from_orcid()
    # print(access_token)

    # get the record of a specific ORCID ID
    # orcid_id = '0000-0002-0491-6724'
    # record = orcid_client.get_record(access_token, orcid_id)
    # print(record)
