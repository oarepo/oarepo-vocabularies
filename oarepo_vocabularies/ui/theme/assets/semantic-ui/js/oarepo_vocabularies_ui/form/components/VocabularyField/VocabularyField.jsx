...
export const VocabularyField = forwardRef(
  (
    ...
    // Initial options come from the record's UI serialization (record.ui.<path>),
    // which carries the resolved { id, title_l10n } for the stored value. The UI
    // path mirrors the field path with the leading `metadata.` replaced by `ui.`.
    const uiFieldPath = `ui.${fieldPath.replace(/^metadata\./, "")}`;

    const initialSuggestions = hasMultipleItems
      ? getIn(values, uiFieldPath, [])
      : _isEmpty(getIn(values, uiFieldPath, {}))
      ? []
      : [getIn(values, uiFieldPath)];
