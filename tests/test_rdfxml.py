...
#
"""Tests for the application/rdf+xml response handler on the vocabularies resource."""
...
def test_rdfxml_read(client, eng_with_mapping):
    response = client.get("/api/vocabularies/languages/eng", headers={"Accept": "application/rdf+xml"})
...
def test_rdfxml_read_via_ui_content_negotiation(client, eng_with_mapping):
    ...
    response = client.get(redirect_response.location, headers={"Accept": "application/rdf+xml"})
...
def test_rdfxml_search(app, db, cache, lang_type, vocab_cf, client, search_clear, clear_vocabulary_permissions):
    ...
    response = client.get("/api/vocabularies/languages", headers={"Accept": "application/rdf+xml"})
